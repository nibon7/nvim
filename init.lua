...
require('lazy').setup({
  ...
  { -- Fuzzy Finder (files, lsp, etc)
    ...
    config = function()
      ...
      vim.keymap.set('n', '<leader>sp', builtin.spell_suggest, { desc = '[S]earch s[P]ell suggestions' })
      ...
      vim.api.nvim_create_autocmd('LspAttach', {
        ...
        callback = function(event)
          ...
          if client and client:supports_method('textDocument/inlayHint', event.buf) then
            ...
            if not vim.lsp.inlay_hint.is_enabled { bufnr = event.buf } then vim.lsp.inlay_hint.enable(true, { bufnr = event.buf }) end
          end
        end,
      })
